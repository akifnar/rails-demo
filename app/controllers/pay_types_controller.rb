class PayTypesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @pay_type.update(pay_type_params)
        format.html { redirect_to @pay_type, notice: "Pay type was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to pay_types_path, notice: "Pay type was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def pay_type_params
      params.require(:pay_type).permit(:id, :name)
    end
end
